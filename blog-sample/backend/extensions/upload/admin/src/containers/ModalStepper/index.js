...
const ModalStepper = ({
  ...
  const handleSubmitEditExistingFile = async (
    ...
    if (file?.type?.includes('image')) {
      ...
      const imageInfo = (await axios.post('http://192.168.103.36:5000/image-captioning', internalFormData)).data.text
      // update caption
      set(fileInfo, 'caption', imageInfo);
    }
    ...
    const requests = filesToUpload.map(
      async ({ file, fileInfo, originalName, originalIndex, abortController }) => {
        ...
        if (file?.type?.includes('image')) {
          ...
          internalFormData.append('file', file);
          const imageInfo = (await axios.post('http://192.168.103.36:5000/image-captioning', internalFormData)).data.text
          // const imageInfo = await request(`https://jsonplaceholder.typicode.com/photos/${n}`, { method: 'GET' });
